...
class Auth extends BaseApi
{
    ...
    public function index(Request $request)
    {
        ...
        //查询数据
        $auth = \app\common\model\Auth::field('id,auth_name,pid,pid_path,auth_c,auth_a,is_nav,level')->where($where)->select();
        //转化为标准的二维数组
        $list = (new Collection($auth))->toArray();
//        $list = $auth->toArray();
//        dump($list);
//        die();
    ...
    public function update(Request $request, $id)
    {
        ...
        //参数检测
        //使用验证器验证，创建验证器对象
        $validate = new \app\adminapi\validate\UserValidate();
//        $validate = new RoleValidate();
        if (!$validate->check($params)) {
            $this->fail($validate->getError(), 401);
        }
        /*$validate = $this->validate($params, [
            'auth_name|权限名称' => 'require',
            'pid|上级权限'       => 'require',

...
class Type extends BaseApi
{
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            //批量添加 规格名称
//            \app\common\model\Spec::create($data, true);
            ...
            //批量添加规格值
//            \app\common\model\SpecValue::create($spec_values, true);
            ...
            \think\Db::commit();
            //返回数据
            $info = \app\common\model\Type::find($type['id']);
            $this->ok($info);
        } catch (Exception $e) {
            //回滚事务
            \think\Db::rollback();
    ...
    public function read($id)
    {
        ...
        $info = \app\common\model\Type::with('specs,specs.specValues,attrs')->find($id);
        //返回数据
        $this->ok($info);
        //相当于以下代码--不使用关联模型
